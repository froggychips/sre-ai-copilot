...
Применять ДО выката кода: без таблицы apply отказывает `execute_error`
(fail-closed, записи в кластер нет). Downgrade — только вместе с откатом
кода на версию без таблицы; JSON-ключи код пишет по-прежнему, так что
откат теряет лишь историю попыток.
"""
