...
class GoldenCase:
    ...
    Поля YAML:
      ...
      expect:    facts / playbooks / cause_contains / cause_not_contains /
                 resolution / intent_action / gate
    """
...
def check_playbooks(case: GoldenCase, store, expected: List[str], result: CaseResult) -> None:
    """Какие v2-playbook-и отбирает matcher по алерту и фактам кейса.

    Детерминированно, без LLM: playbook-и тестируются на тех же
    зафиксированных инцидентах, что и факты, — регресс в preconditions
    ...
    if actual != want:
        result.notes.append(f"playbooks: ожидали {want}, получили {actual}")
...
async def run_case(case: GoldenCase) -> CaseResult:
    ...
    if "playbooks" in expect:
        check_playbooks(case, store, expect["playbooks"], result)
