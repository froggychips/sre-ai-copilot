...
_Ничего в очереди — см. [1.0.0-rc.36] ниже._
...
- Прежние ограничения rc.35 в силе: охват сканирования 16 неймспейсов из 98
  прикладных, и порог `anomaly_signal_health` 25% ждёт эффекта чистки
  baseline.
...
- **Охват сканирования: 16 неймспейсов из 98 прикладных.** Замер: проход по
  16 занимает 219,8 секунды при пике 537Mi из 3Gi. Расширить правкой списка
  нельзя — `sync_topology` собирает `deploys_cache` по всем неймспейсам в
  память до начала обработки и держит его два прохода, так что рост охвата в
  шесть раз умножает и время, и этот кэш. Нужна обработка порциями или
  разбиение задачи по realm.
