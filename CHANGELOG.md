...
- **Владелец стенда: занятие кнопкой ГД теперь считается заявкой.** В резолв
  `kg_namespaces.owner_*` добавлен путь `gd_claim` — кто нажал «Сквад-окружение:
  занять / освободить» (номер стенда кнопка отдаёт как `RESOLVED_SQUAD` в
  resulting-properties). Приоритет: ниже `jira_assignee` (решение 1.0.13
  сохранено), выше `deployed_by`. Лейбл `deployed-by` остаётся от прежнего
  деплойера, пока новый владелец не катал полный деплой сам, — из-за этого два
  стенда две недели числились за человеком, который их освободил, и жалоба
  звучала как «дашборд не обновляется» (обновлялся, врала атрибуция).
- **Дашборд WO-11335: колонка «Установка / Rebuild» видит занятие кнопкой.**
  ...
  так что занятие через неё было невидимо целиком; показывался прошлый Install
  или пусто. Добавлен отдельный оконный скан кнопки (`Claim`), в колонке
  побеждает более свежая запись. Легенда «Занявшего» переписана под реальную
  цепочку резолва.
