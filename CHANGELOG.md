...
- **`live_rca_dataset.py`: контекст на момент инцидента.** Контекст берётся
  ...
  - Новая подкоманда `kg-context` дописывает в уже собранный `cases.jsonl`
    события подов (последнее на под+reason), алерты namespace и деплои. Метка
    `context=kg_reconstructed`.
  - Верхняя граница окна — начало разбора медика: события починки в кейс
    не попадают.
  - `run` накладывает контекст на ctx правил и пишет в results `had_snapshot`
    и `context`.
  - На датасете 24.09 (31 кейс): контекст из графа нашёлся у 29 кейсов.
    Кейсов хотя бы с одним наблюдаемым фактом стало 5 против 0 при разборе
    по одному алерту.
