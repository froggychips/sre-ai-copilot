...
## Phase 1 — Snapshot Integrity + Data Firewall (3–7 days) ✅ COMPLETED
**Owner:** Backend Team  
**Dependencies:** Phase 0

### Tasks
- [x] Finalize immutable snapshot field list.
- [x] Add `observability_fingerprint` to snapshot payload.
- [x] Add `environment_fingerprint` to snapshot payload.
- [x] Implement Snapshot Integrity Gate:
  - FAIL -> block
  - DEGRADED -> low-fidelity only
  - PASS -> full replay
- [x] Implement Replay Data Firewall:
  - snapshot-only inputs
  - no live external calls during replay
- [x] Add integration tests for all gate branches.
