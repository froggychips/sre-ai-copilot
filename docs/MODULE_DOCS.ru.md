...
- **Здоровье Orleans-силоса (v1.0.9)**: discovery `_q_orleans_namespaces()` + шесть запросов `_q_ns_orleans_*` на Orleans-namespace, все только по pull-сериям (`job=~".+/.+"`, `_ORLEANS_PULL`; пуш-копии в push-gateway отсекаются) → `orleans_latency_avg_ms` (взвешенная по вызовам), `orleans_timedout_rate`, `orleans_messaging_fault_rate` (rejected+expired+sent_failed+sent_dropped, без rerouted), `orleans_pings_missed_rate`, `orleans_activation_churn` (в минуту); `_orleans_metrics()` превращает отсутствующие серии сбоев в `0.0`, если есть `latency_count`. Stats: `orleans_namespaces`, `orleans_queries`.
