...
- `Service.node_kind` — роль узла: `service` (k8s Service / логическая точка входа), `workload` (Deployment/StatefulSet/DaemonSet), `ingress` (synthetic). Уникальный ключ — `(namespace, name, node_kind)`. Поиск узла по `(namespace, name)` без типа неоднозначен: `.one_or_none()` бросит `MultipleResultsFound`.
...
- `app/knowledge_graph/k8s_topology_resources_sync.py` (Wave 7-X): каждые 15 минут `kubectl get services/ingresses -A -o json` → edges `serves_traffic` (Service → backing **workload** по selector-match) и `routes_to` (Ingress-как-ресурс → backend Service). Workload — отдельный узел `node_kind='workload'`, синк заводит его сам; матчатся Deployment, **StatefulSet и DaemonSet** (без последних двух 2231 Service за тик уходил в `skipped_no_match` — это все `*-db`). Коммит батчами по 200: одна транзакция на 4200+ upsert-ов держала ACCESS SHARE 12-13 минут и блокировала миграции. Параллельный slice к `k8s_ingress_sync` (Ingress-as-host). RBAC: cluster-role требует `services`, `ingresses`, а также `deployments`/`statefulsets`/`daemonsets` на verbs `get`/`list`/`watch` — проверять через `kubectl auth can-i`, а не по манифесту.
...
- **Здоровье Orleans-силоса (v1.0.9)**: discovery `_q_orleans_namespaces()` + семь запросов `_q_ns_orleans_*` на Orleans-namespace, все только по pull-сериям (`job=~".+/.+"`, `_ORLEANS_PULL`; пуш-копии в push-gateway отсекаются) → `orleans_latency_avg_ms` (взвешенная по вызовам), `orleans_timedout_rate`, `orleans_messaging_fault_rate` (rejected+expired+sent_failed+sent_dropped, без rerouted), `orleans_pings_missed_rate`, `orleans_activation_churn`, `orleans_rerouted_rate` (в минуту; рераны отдельно от сбоев с 1.0.11); `_orleans_metrics()` превращает отсутствующие серии сбоев в `0.0`, если есть `latency_count`. Stats: `orleans_namespaces`, `orleans_queries`.
