...
6. **Operational memory** (same timeline, kinds `evidence`, `diagnosis`, `decision`, `action.applied`, `verification`): what the copilot *did* with the incident, read from `incidents.analysis` (facts, cause / triage_note, executor_applied, executor_verification) and `kg_remediation_decisions`, joined through `fingerprint == incidents.incident_id`. Epistemic per kind: evidence `observed` when any fact was found, diagnosis `inferred` (a conclusion), decision `declared` (policy result), action `observed` (kubectl result), verification `observed` for `verified`/`failed` and `unknown` otherwise. `memory` summarises `records, diagnosis, resolution_quality, decisions, actions, verification, identity_check, outcome` where `outcome ∈ action_<verification> | action_applied_unverified | resolved_without_action | resolved_without_analysis | open_without_action | open_without_analysis`. No analysis rows at all → an `unknowns` entry: "the path was not run" is a different answer from "nothing was done".
