...
6. **Operational memory** (same timeline, kinds `evidence`, `diagnosis`, `decision`, `action.applied`, `action.state_unknown`, `verification`): what the copilot *did* with the incident, read from `incidents.analysis` (facts, cause / triage_note), `kg_remediation_attempts` (applied action and its verification; for records made before that table — `executor_applied` / `executor_verification` in `incidents.analysis`) and `kg_remediation_decisions`, joined through `fingerprint == incidents.incident_id`. Epistemic per kind: evidence `observed` when any fact was found, diagnosis `inferred` (a conclusion), decision `declared` (policy result), action `observed` (kubectl result), `action.state_unknown` `unknown` (stale claim: whether the write reached the cluster is unknown, a human takes over), verification `observed` for `verified`/`failed` and `unknown` otherwise. `memory` summarises `records, diagnosis, resolution_quality, decisions, actions, verification, identity_check, outcome` where `outcome ∈ action_<verification> | action_state_unknown | action_applied_unverified | resolved_without_action | resolved_without_analysis | open_without_action | open_without_analysis`. No analysis rows at all → an `unknowns` entry: "the path was not run" is a different answer from "nothing was done".
7. **Noise.** `kg_incidents.noise = true` when **every** alert of the incident was classified as noise by enrichment (`/enrich-and-forward`: `gen_mismatch_noise`, `meta_noise`, `rollout_noise`, or a `SUPPRESS_ROLLOUT` dedup decision); per-alert kinds live in `extras.noise_fingerprints`. `attach_alert` recomputes the flag — a real alert clears it. Noise incidents stay in the table (they happened) and are reachable by id; `GET /kg/incidents` hides them unless `include_noise=true`. Measured 07.09.2026: 7 of the first 13 incidents and 327 of ~390 weekly alerts were `KubeDeploymentGenerationMismatch`.
8. **Cross-service correlation is out of scope here.** Linking incidents of different services (shared dependency, deploy in window, `calls` edge) is a later roadmap step and will be expressed as edges between incidents, never by relaxing rule 1.

## 12. Blast Radius Contract (`blast_radius.blast_radius_v2`)

"What breaks if X breaks?" — answered with evidence, never as a bare list.

1. **Direction.** The walk goes *against* dependency edges from the target: `calls` (src calls dst), `uses_db`, `uses_nats`, `serves_traffic` (Service node → workload). Up to `max_hops` (default 2); nodes are visited once, by their shortest path.
2. **Evidence per path.** Every edge is classified by `epistemic.classify_edge(sources, last_seen_at, contradictions)` where `sources = extras.discovery_sources ∪ {discovered_by}` and contradictions come from `find_edge_contradictions` (e.g. `serves_traffic` with `endpoints_ready == 0`). An impacted node inherits the **weakest** link of its path; `contradicted` on any link makes the whole path `contradicted`.
3. **Known Unknowns** (`unknowns: [{scope, reason}]`): `callers` — no `calls` edges to the target ("unknown, not absent") or none observed at runtime (all declared/inferred → "list may be incomplete"); `inactive_edges` — edges with `extras.inactive` skipped (count); `hops` / `limit` — how many nodes lie beyond the horizon; `contradicted` — paths that need a human. An empty `impact` with a non-empty `unknowns` is a different answer from "nothing depends on X".
4. **Shape.** Superset of `queries.blast_radius_for` (`services`, `urls`, `*_total`, `*_detailed`, `min_confidence_seen` unchanged) plus `target`, `impact[]` (`service, namespace, node_kind, via, hops, path, epistemic, badge, edge_epistemic, reasons, conflicts, last_seen_at`), `summary` (`impacted_total, by_epistemic, by_via, by_hops, worst_epistemic`). `impact` is sorted strongest evidence first, then nearest. Discord renders the legend with `EPISTEMIC_BADGE` glyphs and up to two `unknowns`.
5. **API.** `GET /kg/blast-radius?namespace=&service=&hops=&limit=&include_inactive=` — router-level `get_current_user`; 404 when the target is not in the graph.

## 13. Remediation Verification Contract (`remediation/verification.py`)

An applied action is not a result. `apply_intent` now answers three questions, each with its own evidence:

1. **Same target?** Before the write, `snapshot_target(intent)` reads the live object (`kubectl get … -o json` via `kubectl_breaker.run_kubectl`, read-only): `uid`, `generation`, `observedGeneration`, `sha256(spec.template)`, replicas. If the incident has a `target_ref` in `kg_remediation_decisions` with a `uid` and it differs from the live `uid`, the write is **refused** (`target_reincarnated:*`, audit `EXECUTOR_APPLY_REFUSED_TARGET_REINCARNATED`). No uid to compare or no live snapshot → the write proceeds and `executor_applied.identity_check` says `unknown:*`.
2. **Did it take effect?** A second snapshot right after the write; both are stored as `executor_applied.target_before` / `target_after` (`TargetSnapshot.to_dict()`; `unknown=True, reason` when unavailable).
3. **Did it help?** `schedule_verification` enqueues the Celery task `remediation_verify` (`apply_async(countdown=…)`, delays from `REMEDIATION_VERIFY_DELAYS_SEC`, default `300,900`; `REMEDIATION_VERIFY_ENABLED` to switch off). `verify_remediation` re-snapshots and `assess`es: `same_identity` (uid unchanged since the write), `action_took_effect` (restart → template hash / generation changed; scale → `spec.replicas == params.replicas`), `converged` (`observedGeneration ≥ generation`), `healthy` (`ready ≥ desired`, `desired > 0`), `alert_resolved` (`kg_alerts.fingerprint == incident_id`), `new_crash_events` (`kg_pod_events` BackOff/CrashLoop/OOM after `applied_at`).

Outcomes: `verified` (took effect, converged, healthy, alert resolved or unknown) → `executor_state = verified`; `failed` (new crash events, action had no effect, or still unhealthy / alert firing on the last attempt) → `executor_state = verification_failed`; `pending` → re-scheduled with the next delay, state unchanged; `unknown` (no live snapshot, uid changed after the action, nothing applied) → recorded, state unchanged. Every check is `True | False | None` — `None` is "could not check", never "ok". Result and reasons live in `analysis["executor_verification"]` (+ `_history`), audit `EXECUTOR_VERIFY_<OUTCOME>`, metric `remediation_verification_total{outcome}`. Verification never fails or blocks `apply_intent`: snapshot errors degrade to `unknown`, a dead broker yields `verification.scheduled=false`.
