...
`serves_traffic` — это ребро Service → его собственный backing workload,
то есть связь узла со своей же реализацией, а не с другим сервисом.

Пока типа узла не было, такое ребро вырождалось в self-loop и отбрасывалось.
С `node_kind` (2.4) оно стало настоящим — и разом сняло признак orphan с 1506
сервисов, не добавив ни одной новой интеграции. Замер на проде 08.08.2026
сразу после выката:

| считаем | orphan |
|---|---|
| любое ребро (как было) | 2072 из 4933 → **42.0%** |
| без `serves_traffic` | 3578 из 4933 → **72.5%** |

Вторая строка совпадает со значением ДО выката: межсервисная связность не
изменилась вообще. Метрика не должна улучшаться от того, что мы поменяли
схему хранения, поэтому `compute_orphan_stats` исключает `serves_traffic`
из признака связности.

Практический смысл: `orphan_pct` отвечает на вопрос «сколько сервисов
изолированы от остальных», а не «сколько узлов имеют хоть одно ребро».
